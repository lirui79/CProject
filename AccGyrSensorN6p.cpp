...
int  AccGyrSensor::processData(RxBuffer *prx_buffer , sensors_event_t* data, int count)
{
    if (processDataCache(prx_buffer) == 0)
       return 0 ;
    ...
    qua_bias[3] = 0;
    VRNavigation::Mathf::qua_norm(qua_bias, qua_bias);
    ...
    for (int j = 0 ; count && j < numSensors ; ++j)
    {
        if (mEnabled[0] == 0 && mEnabled[1] == 0) 
            continue ;
        mPendingEvents[j].timestamp = static_cast<uint64_t>(t_acc + (t_gyr - t_acc) * 0.5);
        if ((j % numSensors) == 0)
        {
            mPendingEvents[j].acceleration.v[0] = acc.vec[0] * 1000.f;
            mPendingEvents[j].acceleration.v[1] = acc.vec[1] * 1000.f;
            mPendingEvents[j].acceleration.v[2] = acc.vec[2] * 1000.f;
            VRNavigation::Mathf::vec_rotate(mPendingEvents[j].acceleration.v, qua_bias, mPendingEvents[j].acceleration.v); 
        } else {
	        mPendingEvents[j].gyro.v[0] = gyr.vec[0];
	        mPendingEvents[j].gyro.v[1] = gyr.vec[1];
	        mPendingEvents[j].gyro.v[2] = gyr.vec[2];
	        VRNavigation::Mathf::vec_rotate(mPendingEvents[j].gyro.v, qua_bias, mPendingEvents[j].gyro.v);		        
        }
...
int  AccGyrSensor::readData(sensors_event_t* data, int count)
{
    const int inter = 100;
    ...
    tx_buffer.cmd = CMDS::READ;
    if (ioctl(dev_fd, SPI_IOC_TXRX, &ioc_arg) < 0)
		ALOGE("ioctl TxRx READ(cmd=0) error");
    else 
		numEventReceived = processData(&rx_buffer , data , count);
